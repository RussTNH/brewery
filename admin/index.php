...
$beers = json_decode(file_get_contents(__DIR__.'/../data/beers.json'), true) ?: [];
...
<section class="panel"><div class="top"><h2>Events</h2><a class="button" href="event.php">+ Add event</a></div><?php foreach($events as $i=>$event): ?><div class="row"><strong><?=htmlspecialchars($event['title'] ?? '')?></strong><br><span class="muted"><?=htmlspecialchars($event['status'] ?? '')?> · <?=htmlspecialchars($event['description'] ?? '')?></span><div class="actions"><a href="event.php?id=<?=$i?>">Edit</a></div></div><?php endforeach; ?></section>
</main></body></html>
