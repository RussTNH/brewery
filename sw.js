const CACHE='clevedon-brewery-v6'
const APP_SHELL=['/','/index.html','/manifest.webmanifest']
async function buildIndex(response){
  const html=await response.text()
  const [ipa,bs21,logo]=await Promise.all([
    fetch('/assets/ipa.b64').then(r=>r.text()),
    fetch('/assets/bs21.b64').then(r=>r.text()),
    fetch('/assets/logo.b64').then(r=>r.text())
  ])
  const out=html
    .replace(/<img class="beer-art" alt="Clevedon Brewery IPA artwork"[^>]*>/,'<img class="beer-art" alt="Clevedon Brewery IPA artwork" src="data:image/webp;base64,'+ipa+'">')
    .replace(/<img class="beer-art" alt="Clevedon Brewery BS21 artwork"[^>]*>/,'<img class="beer-art" alt="Clevedon Brewery BS21 artwork" src="data:image/jpeg;base64,'+bs21+'">')
